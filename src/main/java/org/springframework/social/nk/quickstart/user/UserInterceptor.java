 ...
 */
package org.springframework.social.nk.quickstart.user;
 ...
 * 1. sets the current User in the {@link SecurityContext} from a cookie, if present and the user is still connected to Nk.
...
public final class UserInterceptor extends HandlerInterceptorAdapter {
	...
	private void handleSignOut(HttpServletRequest request, HttpServletResponse response) {
		if (SecurityContext.userSignedIn() && request.getServletPath().startsWith("/signout")) {
			connectionRepository.createConnectionRepository(SecurityContext.getCurrentUser().getId()).removeConnections("nk");
			userCookieGenerator.removeCookie(response);
			SecurityContext.remove();			
		}
	}
	...
	private boolean userNotFound(String userId) {
		// doesn't bother checking a local user database: simply checks if the userId is connected to Nk
		return connectionRepository.createConnectionRepository(userId).findPrimaryConnection(Nk.class) != null;
	}
